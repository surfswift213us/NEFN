...
func _parse_token(p_token):
	var decoded = _jwt_unpack(str(p_token)) # Convert to string
	if decoded.is_empty():
		_valid = false
		return
	_valid = true
	_token = str(p_token) # Convert to string
...
func _parse_refresh_token(p_refresh_token):
	var decoded = _jwt_unpack(str(p_refresh_token)) # Convert to string
	if decoded.is_empty():
		return
	_refresh_expire_time = int(decoded.get("exp", 0))
	_refresh_token = str(p_refresh_token) # Convert to string
...
func _jwt_unpack(p_token : String) -> Dictionary:
	...
	var unpacked = Marshalls.base64_to_utf8(str(payload)) # Convert to string
